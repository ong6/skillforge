...
test('missing or symlinked initialized state cannot be reset by operations or init', async t => {
  ...
  const name = { fielddeck: 'workspace.json', skillforge: 'state.json', proofpack: 'library.json' }[product.name];
...
if (product.name === 'fielddeck') test('headless deck lifecycle, dry run, concurrent processes, private export and UI parity', async t => {
  const o = await fixture(t); await product.init(o); const call = async (n, i) => (await product.execute(n, i, o)).data;
